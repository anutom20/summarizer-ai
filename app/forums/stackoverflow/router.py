...
from app.config import (
    STACKOVERFLOW_ANSWERS_FILTER_VALUE,
    STACKOVERFLOW_QUESTIONS_FILTER_VALUE,
    STACKOVERFLOW_COMMENTS_FILTER_VALUE,
)
...
def get_questions_from_queries(search_term: str):
    try:
        questions = SITE.fetch(
            ...
            q=search_term,
            sort="relevance",
            filter=STACKOVERFLOW_QUESTIONS_FILTER_VALUE,
            pagesize=20,
            page=1,
        )
        return questions
        ...
        logger.error(
            f"An exception occured while getting stackoverflow questions = {e}"
        )
        raise HTTPException(
...
def get_answers_from_question(question_id: int):
    try:
        answers = SITE.fetch(
            ...
            ids=[question_id],
            filter=STACKOVERFLOW_ANSWERS_FILTER_VALUE,
        )
        return answers
...
async def generate_answers_summary(question_id: int):
    try:
        ...
        comments = SITE.fetch(
            ...
            ids=answer_ids,
            filter=STACKOVERFLOW_COMMENTS_FILTER_VALUE,
        )

        gemini_prompt_string = get_stack_exchange_prompt_single_question(
            answers=answers["items"], comments=comments["items"]
        )
